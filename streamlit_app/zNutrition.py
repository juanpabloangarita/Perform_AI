    ...
    with col1:
        ...
        if input_method == "Type Food Item":
            ...
            if barcode:
                ...
                food_columns = [
                    'product_name', 'generic_name', 'product_quantity', 'product_quantity_unit','quantity', 'stores', 'brands', 'allergens_tags',
                    'categories_lc', 'categories_tags', 'data_quality_warnings_tags', 'expiration_date', 'food_groups',
                    'food_groups_tags', 'image_front_small_url', 'ingredients', 'ingredients_hierarchy', 'ingredients_tags',
                    'ingredients_text', 'labels', 'lang', 'nutriments', 'nutriscore_2021_tags', 'nutrition_data_per', 'pnns_groups_1'
                    ]
                real_columns = []
                ...
                selected_meal = st.selectbox("Select Meal", options=meal_options)
                nutritional_info = {
                    'Food': product_df['product_name'],  # Accessing directly from the DataFrame
                    'Units': number_of_units,
                    'Grams per Unit': grams_per_unit,
                    'Total Grams': total_grams,
                    'Calories': product_df['nutriments'][0]['energy-kcal_100g'] * scaling_factor,
                    'Fat': product_df['nutriments'][0]['fat_100g'] * scaling_factor,
                    'Carbohydrates': product_df['nutriments'][0]['carbohydrates_100g'] * scaling_factor,
                    'Sugars': product_df['nutriments'][0]['sugars_100g'] * scaling_factor,
                    'Protein': product_df['nutriments'][0]['proteins_100g']  * scaling_factor
                }
            # Display added foods per meal
            food_log_df = load_food_log('data/processed/csv/')
            food_log_mask = food_log_df['Timestamp']==GIVEN_DATE
